...
func (c *Checker) Check(begin, until time.Time, offlineView offline) ([]Diff, error) {
	...
	events := c.versions.SwitchEvents(begin, until)
	bounds := make([]time.Time, 0, len(events)+1)
	bounds = append(bounds, begin)
	for _, event := range events {
		bounds = append(bounds, event.At)
	}
	diffs := make([]Diff, 0)
	if len(events) == 1 {
		for _, window := range Partition(begin, until, c.step) {
			diffs = append(diffs, c.compareWindow(window.Begin, window.End, window.Index, offlineView)...)
		}
		return diffs, nil
	}
	for window := 0; window+1 < len(bounds); window++ {
		diffs = append(diffs, c.compareWindow(bounds[window], bounds[window+1], window, offlineView)...)
	}
	return diffs, nil
}
